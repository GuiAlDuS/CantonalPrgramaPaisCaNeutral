...
La tabla contiene datos del inventario de emisiones de gases de efecto invernadero realizado en los cantones de **Belén**, **San José**, **Golfito**, **La Unión**, **Desamparados** y **Monteverde**. 

Más información sobre el proyecto que generó estos datos puede accederse en el sitio web de la DCC. Los paquetes que se utilizarán en el análisis son:

```{r message=FALSE, warning=FALSE} 
library(tidyverse) 
``` 

## Lectura de la tabla de datos La tabla con los datos de los cantones pueden
accederse
[acá](https://raw.githubusercontent.com/GuiAlDuS/CantonalProgramaPaisCaNeutral/master/TablaGeneralCantones.csv).
...
Los gráficos se generarán con la libraría Altair, utilizando la interface de R para Python **reticulate**. Altair es una excelente librería para crear gráficos interactivos. La guía para la instalación de Altair en R la pueden encontrar acá. El mapa con ubicación de los cantones se hará utilizando datos geográficos de la Infraestructura de Datos Espaciales de Costa Rica (SNIT): 
...
Según vemos en el mapa y en los gráficos los cantones participantes presentan una gran variabilidad en cantidad y fuentes de emisión. Conociendo las características de estos cantones podemos decir que hay cantones netamente urbanos con alta densidad de población como San José, como también cantones con población dispersa que se dedica en su mayoría a actividades agrícolas como Golfito. Dada esta variabilidad de actividades es interesante hacer el cálculo de emisiones condicionadas tanto por población como por área de cada cantón. Para calcular el aporte de emisiones directas (Alcance 1) por persona utilizaremos los datos del último censo nacional de Costa Rica realizado en el 2011. Estos datos fueron descargados del sistema de consultas del INEC. 

El archivo con el reporte del INEC en formato csv puede descargarse de  [este enlace](https://raw.githubusercontent.com/GuiAlDuS/CantonalProgramaPaisCaNeutral/master/pobCenso2011.csv).
  ...
  encode( 
    ...
    tooltip = list( 
      ...
      alt$Tooltip(field = "Alc1Persona", 
                  type = "quantitative", 
                  title = "Alcance 1 / habitante (Ton. de CO2)", 
                  format = ".3s") 
      ) 
    )$
  configure_legend(labelLimit= 0) 
...
Notemos que al condicionar por área (toneladas de emisiones por hectárea) cantón como emisor cambia si tomamos en cuenta su área. Belén, al ser un cantón de menor extensión, ocupa el primer lugar en *emisiones por hectárea* vs Golfito, que por ser un cantón tan grande, emite valores cercanos a cero.